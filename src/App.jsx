...
// WICHTIG: Das ist die Korrektur, damit dein Foto 'felix.jpg' 
// aus dem src-Ordner wirklich geladen wird.
import felixFoto from './felix.jpg';
...
const App = () => {
  ...
  return (
    <div className={`min-h-screen ${bgColor} font-sans text-gray-800 scroll-smooth`}>
      {/* Navigation */}
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-orange-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <a href="https://wa.me/524422712910" className={`${accentColor} text-white px-5 py-2 rounded-full text-sm font-bold hover:shadow-lg transition-all shadow-md`}>
            Erstgespräch buchen
          </a>
        </div>
      </nav>
      ...
      {/* Über Uns Section */}
      <section id="uber-uns" className="py-24 px-4 bg-white/50">
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="relative">
              <div className="aspect-[4/5] bg-white rounded-3xl overflow-hidden shadow-2xl border-8 border-white">
                 {/* Hier nutzen wir jetzt dein echtes Foto felixFoto */}
                 <img 
                  src={felixFoto} 
                  alt="Felix Schmidt - Qronnect Gründer" 
                  className="w-full h-full object-cover"
                  onError={(e) => {
                    // Fallback-Bild falls felix.jpg doch fehlt
                    e.target.src = "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=1974";
                  }}
                />
              </div>
              <div className="absolute -bottom-6 -right-6 bg-white p-6 rounded-2xl shadow-xl max-w-[200px] border border-orange-50">
                <p className="text-[#8B4513] font-bold italic">"Ich kenne die Stadt nicht nur, ich lebe sie."</p>
              </div>
            </div>
            <div>
              <h2 className="text-4xl font-black mb-6 text-[#4A2C2A]">Willkommen bei Qronnect</h2>
              ...
              <p className="text-gray-600 mb-6 leading-relaxed">
                Hallo, ich bin Felix! Vor zwei Jahren führte mich mein Weg als Ingenieur nach Querétaro. Als Ingenieur schätze ich Präzision – als leidenschaftlicher Tänzer die mexikanische Lebensfreude.
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                {["Bestens vernetzt", "2+ Jahre Erfahrung", "Integrationsexperte", "Local Insider"].map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <CheckCircle2 className="text-green-600 w-5 h-5 flex-shrink-0" />
                    <span className="text-sm font-bold text-[#4A2C2A]">{item}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8 text-left">
            {packages.map((pkg, idx) => (
              <div key={idx} className="bg-[#FFF9F2] rounded-[2rem] p-8 border border-orange-100 flex flex-col h-full hover:shadow-2xl transition-all group">
                <div className={`${accentColor} w-16 h-16 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-lg`}>
                  {pkg.icon}
                </div>
                <h3 className="text-xl font-bold mb-2 text-[#4A2C2A]">{pkg.title}</h3>
                <span className="text-[10px] font-black text-orange-700 tracking-[0.2em] mb-4 uppercase bg-orange-100 px-3 py-1 rounded-full inline-block">
                    {pkg.type}
                </span>
                <p className="text-gray-600 mb-6 flex-grow">{pkg.description}</p>
                <div className="mt-auto pt-6 border-t border-orange-100">
                  <p className="text-xs text-gray-400 mb-2 font-bold uppercase">{pkg.details}</p>
                  <p className="text-2xl font-black text-[#8B4513] mb-6">{pkg.price}</p>
                  <a href="https://wa.me/524422712910" className="block w-full py-4 bg-[#4A2C2A] text-white text-center rounded-xl font-bold hover:bg-[#D2691E] transition-all shadow-md">
                    Jetzt anfragen
                  </a>
                </div>
              </div>
          ...
          <div className="space-y-4">
            {faqs.map((faq, idx) => (
              <div key={idx} className="bg-white p-8 rounded-2xl shadow-sm border border-orange-50">
                <h4 className="font-bold text-lg mb-3 text-[#8B4513]">{faq.q}</h4>
      ...
      <section id="kontakt" className="py-24 px-4 bg-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid sm:grid-cols-2 gap-8 mb-16">
            <div className="bg-[#FFF9F2] p-8 rounded-3xl border border-orange-100 shadow-sm">
              ...
              <a href="mailto:qronnect@outlook.com" className="text-lg font-bold text-[#8B4513] hover:underline break-all">qronnect@outlook.com</a>
            </div>
            <div className="bg-[#FFF9F2] p-8 rounded-3xl border border-orange-100 shadow-sm">
              ...
              <a href="https://wa.me/524422712910" className="text-xl font-bold text-[#8B4513] hover:underline">+52 442 271 2910</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#4A2C2A] text-white py-12 px-4">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-[#4A2C2A] font-bold">Q</div>
            <span className="text-xl font-bold tracking-tight">Qronnect</span>
          </div>
          
          <div className="flex gap-6 opacity-60">
            <Instagram className="w-6 h-6 cursor-pointer hover:text-orange-400" />
            <ExternalLink className="w-6 h-6 cursor-pointer hover:text-orange-400" />
          </div>
